---
name: audit
description: Audit the whole project's test health and write docs/test-audit.md (run this first after installing review-gate) Use when the user asks to check, inspect or audit the project or its tests (e.g. 「프로젝트 점검해줘」).
---
...
1. Run `review-gate audit --json --run-tests` and read the JSON (runner, test command, per-directory tested/total, untested source files ranked by size × exports, weak tests, CI presence, whether the test suite currently passes). If `--run-tests` takes too long or fails to start, rerun without it and say so.
