...
- **The ledger is part of the repository:** never add `.review-gate/` to `.gitignore` (only `runs/`, `phase` and `stop-strikes.json` are ignored, by the tool). `reviews.jsonl` and `config.json` get committed — they are how findings are counted later. A backfill writes tests; if production code must change to make something testable, say so in the report and keep it minimal — it is a change to review, not a side note.
...
- Commands: `review-gate audit | phase test|implement | review [--worktree|--base B --head H] | lint-tests | red-green --test-cmd "…" | pin-check --test T --target S | env-check --test T`.
