...
function singleTestCmd(repo, testFile, override) {
  ...
  const r = detectRunner(repo).runner;   // 'repo' here is the workspace dir the caller resolved
...
function pinCheck(repoDir, testFile, target, opts = {}) {
  // Monorepos: paths are taken relative to the current directory first (apps/web/…), then to the repository root; the runner is
  // detected and executed where the test lives (its workspace), so `cd apps/web && review-gate pin-check --test src/… --target src/…` works.
  const repo = repoRoot(repoDir || '.'); const cwd = path.resolve(repoDir || '.');
  const resolve = rel => [path.resolve(cwd, rel), path.join(repo, rel)].find(a => fs.existsSync(a)) || null;
  const tAbs = resolve(target), testAbs = resolve(testFile);
  if (!tAbs) return { verdict: 'error', note: `target not found: ${target} (relative to ${cwd} or ${repo})` };
  if (!testAbs) return { verdict: 'error', note: `test not found: ${testFile}` };
  ...
  const testRel = path.relative(ws, testAbs);
  const cmd = singleTestCmd(ws, testRel, opts.cmd); if (!cmd) return { verdict: 'error', note: 'no runner detected — pass --cmd "<runner> {file}"' };
  ...
  const timeout = (opts.timeoutSec || 600) * 1000; const out = { test: testFile, target, cmd, workspace: path.relative(repo, ws) || '.' };
  const restore = () => { try { fs.writeFileSync(tAbs, original); fs.unlinkSync(bak); } catch {} };
  process.on('SIGINT', () => { restore(); process.exit(130); });
  try {
    fs.writeFileSync(tAbs, stub);
    let t0 = Date.now(); const r1 = run(cmd, [], { cwd: ws, shell: true, timeout }); out.blanked_fails = r1.code !== 0; out.blanked_seconds = (Date.now() - t0) / 1000; out.blanked_tail = (r1.out + r1.err).slice(-800);
    restore();
    t0 = Date.now(); const r2 = run(cmd, [], { cwd: ws, shell: true, timeout }); out.restored_passes = r2.code === 0; out.restored_seconds = (Date.now() - t0) / 1000; out.restored_tail = (r2.out + r2.err).slice(-800);
